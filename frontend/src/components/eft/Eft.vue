      ...
      <div class="row row-equal flex align--center justify--center">
        <div class="flex xs7">
         <form @submit.prevent="onSubmit">
          <va-card title="ELECTRONIC FUND TRANSFER">
              <div class="row flex">
...
export default {
  ...
  methods: {
      async  onSubmit(){
        this.bankCodeErrors = this.bank.code != undefined ? [] : ["Choose Bank"]      
        ...
        this.branchCodeErrors = this.branchCode ? [] : ['Branch Code cannot be empty']
        ...
        this.$http.post(`${this.$apiUrl}/jenga/eft/${this.$store.state.activeAccount.accountNumber}`,data)
        .then(data=>{
  ...
  computed : {
     formReady(){
         ...
         !this.branchCodeErrors.length &&
          !this.bankCodeErrors.length
      }
